...
import { 
   JC_CREATE_REQUEST, JC_CREATE_RESET 
...
const CreateJCMasterScreen = ({ history }) => {
   ...
   const jcMasterCreate = useSelector((state) => state.jcMasterCreate)
	const { success, jcMaster, error: errorCreate } = jcMasterCreate
   ...
   useEffect(() => {
      if (!userInfo) {
		...
		if (success) {
         ...
         //console .log(jcMaster)
         //history.push(`/jcmasters/${jcMaster._id}/edit`)
         history.push('/jcmasterlist');
		   dispatch({ type: JC_CREATE_RESET })
         NotificationManager.success(`JC Record ${jcMaster.jcno} has been successfully created !`, 'Successful!', APPLICATION_NOTIFICATION_TIME_OUT);
		}

      if(errorCreate) {
         dispatch({ type: JC_CREATE_RESET })
         NotificationManager.error(`Error in creating JC Record !`, 'Error!', APPLICATION_NOTIFICATION_TIME_OUT);
      }
...
//      logger("*********s. Master Data is autoIncValues ========= ", autoIncValues.SP)
   }
   ...
   const [ proposedMachine,  setProposedMachine ] = useState("FB1 - Flat Bed-1");
   ...
   const handleProdLayoutUOM = (e) => {
      if(e.target.value.trim() === "none") {
         ...
         setbomUOM(e.target.value);
         setbomUOMLabel((naam === undefined || naam[0] === undefined)? "":naam[0].name);
         if(prodLayoutWidth > 0 || prodLayoutHeight > 0) {
            //logger("!!! USER selected UNIT is !!!! ", naam[0].name);
            //logger("!!! PREVIOUS selected UNIT WAS !!!! ", prodLayoutUOMLabel);
   ...
   const calculateBOMArea = (bomunit) => {
     ...
     console.log("bomUOM is ", bomUOMLabel)
      let width = 0;
      let height = 0;
      let area = 0;
      ...
      } else if(prodLayoutUOMLabel === "inch") {
         if(bomunit === "mm") {
            width = ((prodLayoutWidth*25.4)/prodLayoutWidthUps);
            height = ((prodLayoutHeight*25.4)/prodLayoutHeightUps);
         } else if(bomunit === "cm") {
            ...
            width = prodLayoutWidthUps!==0?(prodLayoutWidth / prodLayoutWidthUps):0;
            height = prodLayoutHeightUps!==0? (prodLayoutHeight / prodLayoutHeightUps):0;
         }
      } else if(prodLayoutUOMLabel === "ft") {
         if(bomunit === "mm") {
            width = ((prodLayoutWidth*304.8)/prodLayoutWidthUps);
            height = ((prodLayoutHeight*304.8)/prodLayoutHeightUps);
         } else if(bomunit === "cm") {
   ...
   const findFormErrors = () => {
      ...
      //logger("Now printing inputFields of customer ==== ", inputFields)
      inputFields.map(inputField => {
         if(inputField.customerId.trim().length === 0) {
            newErrors.customerId = 'Select a Customer !'
         }
         if(inputField.customerPrice.trim().length === 0) {
      ...
      <FormContainer>
         <Row>
            ...
            {/*<Col lg={6} md={12} xs={12} style={{textAlign:"end"}}>
               <Link to= '/bulkjcupload'>
                  <Button
                     className='btn-md button-class'
                     variant="primary" 
                  >
         ...
         {loadingMasterData ? (
            ...
            <>
               <FormFieldsContainer frameTitle = "Please Enter JC Details !!!" >
                     ...
                     <Col>
                        ...
                        { inputFields.map(inputField => (
                           <Row key={inputField.id}>
                              ...
                              <Col lg={(inputField.customer!==undefined?(inputField.customer.trim() !== "C0022" ? 4 : 4):4)} md={12} xs={12} style={{textAlign:"end"}}>
                                 <Button
                                    variant="danger" 
                                    className="btn-md"
...
export default CreateJCMasterScreen
