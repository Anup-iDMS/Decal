...
import YieldReportTable from './../../../components/tables/qa/YieldReportTable';
import { getYieldReport } from './../../../actions/production/jobCardActions';


const YieldReportScreen = ({ history }) => {
   ...
   const yieldReport = useSelector((state) => state.yieldReport)
	const { loading, jobCards, error:errorReportLoad } = yieldReport;
   ...
   const submitHandler = (e) => {
      e.preventDefault();
      dispatch(getYieldReport(jcId, startDate, endDate))
   }
      ...
      <React.Fragment>
         ...
         <Row className='align-items-center mx-1'>
            <Col lg={6} md={12} xs={12}>
               <h4 ><i className="fas fa-list"></i> Yield Report</h4>
            </Col>
         </Row>
         <br></br>
         <hr></hr>
         ...
         {loading ? (
            ...
            <>
               {jobCards === undefined? (<p className="text-center"><b>No Records Found</b></p>) : (
                  <YieldReportTable data={jobCards} />
               )}
...
export default YieldReportScreen
