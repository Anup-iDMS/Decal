...
const SalesOrderConfirmationScreen = ({ history, match }) => {
   ...
   //post deleted JC record
   const salesOrderDelete = useSelector((state) => state.salesOrderDelete);
   const { success: successDelete, error: errorDelete } = salesOrderDelete
   console.log("----------- salesOrderDelete ============= ", salesOrderDelete)
   console.log("----------- Delete flag is ============= ", successDelete)
   ...
   useEffect(() => {
      ...
      console.log("??????????? inside useEffect and Delete flag is ============= ", successDelete)
      if(successDelete) {
         history.push('/bookorder');
         //dispatch({ type: SALES_ORDER_UPDATE_RESET })
         NotificationManager.success(`Sales Order # ${salesOrder.soNumber} has been successfully cancelled !`, 'Confirm!', APPLICATION_NOTIFICATION_TIME_OUT);
      }
      if(errorDelete) {
   ...
   const setFormData = () => {
      ...
      console.log("--------> before setting data <----------- ", newSODetails1)
   ...
   const submitHandler = (e) => {
      ...
      dispatch({ type: SALES_ORDER_UPDATE_RESET })
      NotificationManager.success(`Sales Order # ${salesOrder.soNumber} has been successfully updated !`, 'Confirm!', APPLICATION_NOTIFICATION_TIME_OUT);
   }
   ...
   const handleReset = () => {
      ...
      history.push('/salesorderslist');
      //dispatch({ type: SALES_ORDER_UPDATE_RESET })
      NotificationManager.success(`Sales Order # ${salesOrder.soNumber} has been successfully cancelled !`, 'Successful!', APPLICATION_NOTIFICATION_TIME_OUT);
      ...
      <React.Fragment>
         {loading ? (
            ...
            <React.Fragment>
               ...
               <Row>
                  <Col>
                     <h3>Confirm Sales Order Details</h3>
                  </Col>
               </Row>
               <br></br>
               <div style={{border:"1px solid black", borderRadius:"5px", background:"white"}}>
                  <h6 className="px-2 py-2" style={{background:"#e84347", color:"white"}}>Please Confirm Sales Order Details !!!</h6>
                  <br></br>
