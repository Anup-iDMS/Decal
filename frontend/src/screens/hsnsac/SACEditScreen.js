...
import { updateSAC, getSACDetails } from '../../actions/masters/sacActions';

//import Redux "constantc(s)"
import { SAC_UPDATE_RESET } from '../../constants/masters/sacConstants';

import { 
   APPLICATION_NOTIFICATION_TIME_OUT 
} from '../../constants/application/application';

const SACEditScreen = ({ history, location, match }) => {
   ...
   const sacId = match.params.id;
   
   const sacDetails = useSelector((state) => state.sacDetails)

   const { loading, sac, error } = sacDetails;

   //post updated Job Card Record
   const sacUpdate = useSelector((state) => state.sacUpdate);
   const { success: successUpdate, error: errorUpdate } = sacUpdate
   ...
   useEffect(() => {
      ...
      if (sac._id !== sacId) {
         ...
         dispatch(getSACDetails(sacId))
      } else {
         setFormData();
      }

		if (successUpdate) {
         //history.replace(`/sac/${sac._id}/edit`)
         history.push('/saclist');
         dispatch({ type: SAC_UPDATE_RESET })
         NotificationManager.success(`SAC Entry has been successfully updated !`, 'Successful!', APPLICATION_NOTIFICATION_TIME_OUT);
		}
	...
	}, [history, sacId, sac, successUpdate])

   const setFormData = () => {
      setProvisionType(sac.provisionType);
      setHSNCode(sac.hsnCode);
      setIsActive(sac.isActive);
      setGoodsDescription(sac.goodsDescription);
      setSACCode(sac.sacCode);
      setServiceDescription(sac.serviceDescription)
      setGSTRate(sac.gstRate)
      setIGSTRate(sac.igstRate)
      setSGSTRate(sac.sgstRate)
      setCGSTRate(sac.cgstRate);
      setUGSTRate(sac.ugstRate);
   ...
   const submitHandler = (e) => {
      console.log("------------- else sacCode -------------- ", sacCode)
      e.preventDefault();
      ...
      if ( Object.keys(newErrors).length > 0 ) {
         ...
         console.log("------------- else sacCode -------------- ", sacCode)
         console.log("------------- else _id -------------- ", sacId)
         console.log("------------- else serviceDescription -------------- ", serviceDescription)
      ...
      <FormContainer>
         ...
         {loading ? (
            ...
            <>
               <Row>
                  <Col>
                     <h3>Edit SAC Registration</h3>
                  </Col>
               </Row>
               <br></br>
               <FormFieldsContainer frameTitle = "Please Edit SAC Details !!!" >
