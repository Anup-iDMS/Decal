...
import { createHSNSAC } from '../../actions/masters/hsnActions';

//import Redux "constantc(s)"
import { HSN_CREATE_RESET } from '../../constants/masters/hsnConstants';

import { 
   APPLICATION_NOTIFICATION_TIME_OUT 
} from '../../constants/application/application';

const HSNCreateScreen = ({ history, location }) => {
  ...
  // const userDetails = useSelector((state) => state.userDetails)
   ...
   const hsnCreate = useSelector((state) => state.hsnCreate)

   const { success, hsnsac, loading, error: errorCreate } = hsnCreate
   ...
   useEffect(() => {
      if (!userInfo) {
		...
		if (success) {
         ...
         dispatch({ type: HSN_CREATE_RESET })
         NotificationManager.success(`HSN Entry has been successfully created !`, 'Successful!', APPLICATION_NOTIFICATION_TIME_OUT);
		}

      if(errorCreate) {
         dispatch({ type: HSN_CREATE_RESET })
         NotificationManager.error(`Error in creating HSN Entry !`, 'Error!', APPLICATION_NOTIFICATION_TIME_OUT);
      }
      ...
      <FormContainer>
         ...
         {loading ? (
            ...
            <>
               <Row>
                  <Col>
                     <h3>HSN Registration</h3>
                  </Col>
               </Row>
               <br></br>
               <FormFieldsContainer frameTitle = "Please Enter HSN Details !!!" >
...
export default HSNCreateScreen
